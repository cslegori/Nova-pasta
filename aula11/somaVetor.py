...
import numpy as np
    ...
    layout =  [ 
        [sg.Text("Digite a quantidade de numeros que deseja inserir:")],
        [sg.Input(key='N')],
    ...
    while True:
        ...
        try:
            n = int(valores['N'])
            if n <=0:
                sg.popup("Insira somente numeros positivos")
                continue
            break
        except:
    ...
    for i in range(n):
        layout = [
            [sg.Text(f"Digite o {i+1}° numero")],
            [sg.Input(key='Campeao')],
            [sg.Button("Ok"), sg.Button("Cancelar")]
        ]
    janela = sg.Window("Entrada de numero",layout)
